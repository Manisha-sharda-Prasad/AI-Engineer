import React from "react";
export function EditIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M4 16.5V20h3.5L18.3 9.2l-3.5-3.5L4 16.5Zm13.9-9.4 1.4-1.4a1 1 0 0 0 0-1.4l-1.2-1.2a1 1 0 0 0-1.4 0l-1.4 1.4 3.5 3.6Z" />
    </svg>
  );
}
export function RefreshIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M20 11a8 8 0 1 0 2 5.3M20 4v7h-7" />
    </svg>
  );
}
export function CloseIcon() {
  return (
    <svg className="close-icon" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M6 6l12 12M18 6 6 18" />
    </svg>
  );
}
export function LabelIcon({ label }) {
  return label === "watched" ? (
    <svg viewBox="0 0 24 24">
      <path d="m5 12 4 4L19 6" />
    </svg>
  ) : label === "bookmarked" ? (
    <svg viewBox="0 0 24 24">
      <path d="M6 3h12v18l-6-4-6 4V3Z" />
    </svg>
  ) : (
    <svg viewBox="0 0 24 24">
      <path d="M4 7h16M9 7V4h6v3m-8 0 1 13h8l1-13M10 11v5m4-5v5" />
    </svg>
  );
}
export function WorkspaceIcon({ name }) {
  const paths = {
    filter: "M4 5h16M7 12h10m-7 7h4",
    sort: "M6 7h12M6 12h8M6 17h4",
    manual: "M12 5v14M5 12h14",
    ai: "m12 3 1.6 5.4L19 10l-5.4 1.6L12 17l-1.6-5.4L5 10l5.4-1.6L12 3ZM19 15l.7 2.3L22 18l-2.3.7L19 21l-.7-2.3L16 18l2.3-.7L19 15Z",
    info: "M12 17v-6m0-3h.01M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Z",
    edit: "M4 20h4L19 9l-4-4L4 16v4ZM13 7l4 4",
    expand: "M8 14l4-4 4 4M8 10l4-4 4 4",
    collapse: "M8 6l4 4 4-4M8 14l4 4 4-4",
    moveUp: "m7 14 5-5 5 5",
    moveDown: "m7 10 5 5 5-5",
    move: "M5 7h11m0 0-3-3m3 3-3 3M19 17H8m0 0 3-3m-3 3 3 3",
    selectAll: "M5 4h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1Zm3 8 2.5 2.5L16 9",
    deselectAll: "M5 4h14a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1Zm3 8h8",
    outline: "M5 5h4v4H5V5Zm7 1h7M5 13h4v4H5v-4Zm7 1h7M7 9v4",
    menu: "M4 6h16M4 12h16M4 18h16",
    settings: "M12 8.5a3.5 3.5 0 1 0 0 7 3.5 3.5 0 0 0 0-7Zm7.4 3.5a7.8 7.8 0 0 0-.1-1l2-1.5-2-3.4-2.4 1a8 8 0 0 0-1.8-1L14.8 3h-4l-.4 3.1a8 8 0 0 0-1.8 1l-2.4-1-2 3.4 2 1.5a7.8 7.8 0 0 0 0 2l-2 1.5 2 3.4 2.4-1a8 8 0 0 0 1.8 1l.4 3.1h4l.4-3.1a8 8 0 0 0 1.8-1l2.4 1 2-3.4-2-1.5a7.8 7.8 0 0 0 .1-1Z",
    progress: "M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6",
    share: "M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Zm0 0c2.2 2.45 3.3 5.45 3.3 9s-1.1 6.55-3.3 9m0-18C9.8 5.45 8.7 8.45 8.7 12s1.1 6.55 3.3 9M3.4 9h17.2M3.4 15h17.2",
  };
  return (
    <svg viewBox="0 0 24 24">
      <path d={paths[name]} />
    </svg>
  );
}
